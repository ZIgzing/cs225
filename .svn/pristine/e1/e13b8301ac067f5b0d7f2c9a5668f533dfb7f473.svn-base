...
class KDTree
{
  public:
    ...
    Point<Dim> findHelper(const Point<Dim> &query, const Point<Dim> &x, size_t left, size_t right, int ourDim) const;
    Point<Dim> findNearestNeighbor(const Point<Dim> & query) const;

    // functions used for grading:


    size_t partition(size_t left, size_t right, size_t pivotIndex, int ourDim);
    
    void quickSelect(size_t left, size_t right, int dim, size_t middleIndex);

    void buildKDTree(size_t left, size_t right, int dim);

    //int findDistance(const Point<Dim>& first, const Point<Dim>& second) const;


    //Point<Dim> findHelper(size_t left, size_t right, const Point<Dim>& query, Point<Dim> currBest, int dim) const;
