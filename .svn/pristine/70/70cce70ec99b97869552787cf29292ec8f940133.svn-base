...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
  ...
  std::transform(capW1.begin(), capW1.end(), capW1.begin(), toupper);
  std::transform(capW2.begin(), capW2.end(), capW2.begin(), toupper);

  if (dict.find(capW1) != dict.end() && dict.find(capW2) != dict.end()) {
      if (dict.find(capW1)->second == dict.find(capW2)->second) {
        return true;
      }
  }
    /* Your code goes here! */
