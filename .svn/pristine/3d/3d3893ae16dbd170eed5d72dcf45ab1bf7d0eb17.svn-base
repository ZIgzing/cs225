                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(mergeQueue.empty()){
        ...
        if(singleQueue.empty()){
            smallVal = mergeQueue.front();
            mergeQueue.pop();
            return smallVal;
        }
        else{
            smallVal = singleQueue.front();
            if(smallVal->freq.getFrequency() < mergeQueue.front()->freq.getFrequency()){
                singleQueue.pop();
                return smallVal;
            }
            else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for(unsigned int i = 0; i < frequencies.size(); i++){
        TreeNode* lNode;
        lNode = new TreeNode(frequencies[i]);
        singleQueue.push(lNode);
    }
    while(singleQueue.size() + mergeQueue.size() > 1){
        TreeNode* firstRemove = removeSmallest(singleQueue, mergeQueue);
        TreeNode* secondRemove = removeSmallest(singleQueue, mergeQueue);
        unsigned int freqSum = firstRemove->freq.getFrequency() + secondRemove->freq.getFrequency();
        TreeNode* nInternal = new TreeNode(freqSum);
        nInternal->left = firstRemove;
        nInternal->right = secondRemove;
        mergeQueue.push(nInternal);
    }
    TreeNode* temp;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     while(bfile.hasBits()){
      if(bfile.getNextBit() == 1){
        ...
        TreeNode* newInternal = new TreeNode(0);
        newInternal->left = readTree(bfile);
        newInternal->right = readTree(bfile);
        return newInternal;
      }
    }
    /**
