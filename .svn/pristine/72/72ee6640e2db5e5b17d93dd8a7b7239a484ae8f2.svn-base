...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        string mostRecent = ll.customer + ll.url;
        if(pageVisitedTable.keyExists(ll.url) == true){
          pageVisitedTable[ll.url] = true;
        }
        else if(pageVisitedTable.keyExists(ll.url) == false){
          pageVisitedTable.insert(ll.url, true);
          uniqueURLs.push_back(ll.url);
        }
        if(whenVisitedTable.keyExists(mostRecent) == true && whenVisitedTable[mostRecent] <= ll.date){
            whenVisitedTable[mostRecent] = ll.date;
        }
        else if(whenVisitedTable.keyExists(mostRecent) == false){
            whenVisitedTable.insert(mostRecent, ll.date);
        }
    }
    infile.close();
}
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
    ...
    //(void) url;      // prevent warnings... When you implement this function, remove this line.
     ...
     // replaceme
     if(whenVisitedTable.isEmpty()){
       return false;
     }
     if(whenVisitedTable.isEmpty() == false &&  whenVisitedTable.keyExists(combined)){
       return true;
     }
     else{
       return false;
                                  ...
                                  const string& url) const
{
     ...
     * @todo Implement this function.
     */
    string total = customer + url;
  //  (void) customer; // prevent warnings... When you implement this function, remove this line.
    //(void) url;      // prevent warnings... When you implement this function, remove this line.
    if(whenVisitedTable.isEmpty()){
      return false;
    }
    if(whenVisitedTable.keyExists(total) == true){
      time_t ourTime = whenVisitedTable.find(total); // replaceme
      return ourTime;
    }    
    return time_t();
}
