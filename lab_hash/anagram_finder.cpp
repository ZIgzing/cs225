...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
     ...
     */
    Dict<char, int> wHash(64);
    Dict<char, int> tHash(64);
    typename Dict<char, int>::iterator it = wHash.begin();
    typename Dict<char, int>::iterator it1 = wHash.begin();
    if(word.length() != test.length()){
        return false;
    }
    for(unsigned long i = 0; i < word.length(); i++){
        wHash[word[i]]++; // same as wHash[word[1]] = wHash[word[1]] + 1
    }   
    for(unsigned long j = 0; j < test.length(); j++){
        tHash[test[j]]++;
    }
    for(unsigned long k = 0; k < word.length(); k++){
        if(tHash[k] != wHash[k]){
            return false;
        }
    }
